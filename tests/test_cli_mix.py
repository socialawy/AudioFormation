...
def test_mix_command_success(runner, sample_project, isolate_projects):
    ...
    with patch("audioformation.mix.mix_project") as mock_mix, \
...
def test_mix_command_gate_fail(runner, sample_project, isolate_projects):
    ...
    with patch("audioformation.mix.mix_project") as mock_mix, \
         patch("audioformation.pipeline.can_proceed_to", return_value=(False, "Previous step failed")):
