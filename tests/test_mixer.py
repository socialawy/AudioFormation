...
def mock_pydub(monkeypatch):
    ...
    class MockSegment:
        ...
        def get_array_of_samples(self):
            ...
            return array.array('h', [1000] * int(self.duration_ms * self.frame_rate / 1000) * self.channels)
...
class TestAudioMixer:
    ...
    def test_ensure_vad_model_fallback(self):
        ...
        with patch("audioformation.audio.mixer.torch.hub.load", side_effect=Exception("Network error")):
            mixer = AudioMixer({})
            mixer._ensure_vad_model()
            assert mixer.method == "energy"
    ...
    def test_generate_envelope_energy(self, mock_pydub):
        ...
        with patch.object(mixer, "_get_energy_timestamps", return_value=[{'start': 1000, 'end': 2000}]):
