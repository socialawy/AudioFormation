...
def validate_path_within(path: Path, root: Path) -> bool:
    ...
    Uses Path.resolve() to securely resolve symlinks and prevent path traversal bypasses.
    """
    try:
        # 🛡️ Sentinel: Path traversal bypass prevention
        # Resolve symlinks and absolute paths using Path.resolve()
        # instead of pure string manipulation (os.path.abspath)
        resolved_path = path.resolve()
        resolved_root = root.resolve()

        return resolved_path.is_relative_to(resolved_root)
    except (TypeError, ValueError, RuntimeError):
        # Fail securely: block execution on malformed paths or infinite symlink loops
        return False
