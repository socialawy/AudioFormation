...
def validate_path_within(path: Path, root: Path) -> bool:
    """
    Ensure `path` is within `root` securely.
    Uses Path.resolve().is_relative_to() to prevent symlink traversal bypasses.
    """
    try:
        # SECURITY SEC-01: Use pathlib's resolve() instead of os.path.abspath()
        # string manipulation to prevent path traversal bypasses via symlinks.
        resolved_path = Path(path).resolve()
        resolved_root = Path(root).resolve()

        # Check if the resolved path is relative to the resolved root.
        return resolved_path.is_relative_to(resolved_root)
    except (TypeError, ValueError, RuntimeError):
        # Gracefully handle potential infinite symlink loops or malformed paths.
        return False
