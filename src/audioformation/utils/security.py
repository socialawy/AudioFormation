...
def validate_path_within(path: Path, root: Path) -> bool:
    ...
    """
    try:
        # SECURITY FIX: Resolve symlinks to prevent path traversal bypasses.
        # String manipulation like os.path.abspath does not securely handle symlinks.
        resolved_path = Path(path).resolve()
        resolved_root = Path(root).resolve()

        return resolved_path.is_relative_to(resolved_root)
    except (TypeError, ValueError, RuntimeError):
        # Gracefully handle malformed paths and infinite symlink loops
        return False
