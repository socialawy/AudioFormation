...
def validate_path_within(path: Path, root: Path) -> bool:
    ...
    Uses Path.resolve() to securely evaluate symlinks and prevent path traversal bypasses.
    """
    try:
        # Resolve fully evaluates symlinks, catching potential infinite loops or malformed paths
