...
def validate_path_within(path: Path, root: Path) -> bool:
    """
    Ensure `path` is within `root` securely resolving symlinks.
    Prevents path traversal bypasses where an attacker creates a symlink
    inside the safe directory pointing to an outside sensitive file.
    """
    try:
        # Resolve symlinks and normalize path completely
        # This accesses the filesystem to resolve symlinks, which is necessary
        # for true security to prevent symlink-based path traversal.
        ...
        # Gracefully handle potential infinite symlink loops and malformed paths
        return False
