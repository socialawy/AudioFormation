...
async def generate_project(
    ...
    update_node_status(project_id, "generate", "running", engine=engine_name or "per-character")
    ...
    fail_threshold = gen_config.get("fail_threshold_percent", DEFAULT_FAIL_THRESHOLD_PCT)
    overall_fail_rate = (total_fail_chunks / max(total_chunks, 1)) * 100

    if overall_fail_rate > fail_threshold:
        update_node_status(
            project_id, "generate", "failed",
            error=f"Fail rate {overall_fail_rate:.1f}% exceeds threshold {fail_threshold}%",
        )
    else:
...
async def _generate_chapter(
    ...
    if not source_path.exists():
        update_chapter_status(project_id, ch_id, "failed", error="Source file not found.")
        return {"chapter_id": ch_id, "status": "failed", "error": "Source file not found."}
    ...
    try:
        ...
        return {"chapter_id": ch_id, "status": "failed", "error": error_msg}

    # Chunking config
    max_chars = gen_config.get("chunk_max_chars", 200)
    strategy = gen_config.get("chunk_strategy", "breath_group")
    crossfade_ms = gen_config.get("crossfade_ms", DEFAULT_CROSSFADE_MS)
    leading_silence_ms = gen_config.get("leading_silence_ms", DEFAULT_LEADING_SILENCE_MS)
    max_retries = gen_config.get("max_retries_per_chunk", DEFAULT_MAX_RETRIES)
    use_ssml = gen_config.get("edge_tts_ssml", True) and engine.supports_ssml
    ...
    for segment in segments:
        ...
        for chunk_text_item in chunks:
            ...
            for attempt in range(max_retries + 1):
                ...
                if result.success and chunk_path.exists() and chunk_path.stat().st_size > 0:
                    # QC scan this chunk
                    ...
                    if qc_result.status == "fail" and attempt < max_retries:
                        ...
                        click.echo(f"    ⚠ {chunk_id}: QC fail, retry {attempt + 1}/{max_retries}")
                        continue
                    elif qc_result.status == "fail":
                        failed_chunks += 1
                        click.echo(f"    ✗ {chunk_id}: QC fail after {max_retries} retries")
                    ...
                    if attempt < max_retries:
                        click.echo(
                            f"    ⚠ {chunk_id}: attempt {attempt + 1} failed — {last_error}"
                        )
                    continue

            if not success:
                failed_chunks += 1
                click.echo(f"    ✗ {chunk_id}: FAILED — {last_error}")
    ...
    if chunk_paths:
        ...
        if stitch_ok:
            click.echo(f"    ✓ Stitched {len(chunk_paths)} chunks → {ch_id}.wav")
        else:
